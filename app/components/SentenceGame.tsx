...
import { SENTENCE_CONTENT as CONTENT } from '../lib/content';
...
export default function SentenceGame({ profile, userId, onFinish }: { profile: any; userId: string; onFinish: () => void }) {
  ...
  useEffect(() => { timerRef.current = setInterval(() => setTimer(t => t + 1), 1000); sounds.gameStart(); if (questions[0]) setPool(shuffle([...questions[0].words])); return () => { if (timerRef.current) clearInterval(timerRef.current); }; }, []);
  useEffect(() => { if (questions[idx]) setTimeout(() => sounds.speak(questions[idx].correct), 400); }, [idx, questions]);

  const addWord = (w: string, wi: number) => { if (result !== null) return; sounds.tap(); setPlaced(p => [...p, w]); setPool(p => p.filter((_, i) => i !== wi)); };
  const removeWord = (wi: number) => { if (result !== null) return; sounds.tap(); setPool(p => [...p, placed[wi]]); setPlaced(p => p.filter((_, i) => i !== wi)); };
  const check = () => {
    if (result !== null) return;
    const ok = placed.join(' ') === questions[idx].correct; setResult(ok);
    if (ok) { setStreak(s => s + 1); setScore(s => ({ ...s, c: s.c + 1 })); sounds.correct(); } else { setStreak(0); setScore(s => ({ ...s, w: s.w + 1 })); sounds.wrong(); sounds.speak(questions[idx].correct); }
    setTimeout(() => { if (idx + 1 >= questions.length) { if (timerRef.current) clearInterval(timerRef.current); setDone(true); } else { const next = idx + 1; setIdx(next); setPlaced([]); setPool(shuffle([...questions[next].words])); setResult(null); } }, ok ? 700 : 1500);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-5" style={{ background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)' }}>
      <div className="max-w-md mx-auto w-full mb-4">
        ...
        <div className="flex items-center gap-2"><div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}><div className="h-full rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%`, background: 'linear-gradient(90deg,#ef4444,#f97316)' }} /></div><span className="text-xs font-bold text-slate-400">{idx}/{questions.length}</span></div>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
        <div className="text-4xl mb-2">🔧</div>
        <h3 className="text-lg font-black text-white mb-2">Build the Sentence!</h3>
        <button onClick={() => sounds.speak(questions[idx].correct)} className="mb-4 px-5 py-2 rounded-full text-sm font-bold transition-all hover:scale-105" style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: 'white' }}>🔊 Listen</button>
        <div className="w-full min-h-[56px] rounded-2xl p-3 mb-4 flex flex-wrap gap-2" style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${result === true ? '#22c55e' : result === false ? '#ef4444' : 'rgba(255,255,255,0.1)'}` }}>
          ...
          {placed.map((w, i) => <button key={i} onClick={() => removeWord(i)} className="px-3 py-1.5 rounded-lg text-sm font-bold hover:scale-105" style={{ background: 'rgba(99,102,241,0.3)', color: '#c7d2fe', border: '1px solid rgba(99,102,241,0.4)' }}>{w}</button>)}
        </div>
        <div className="flex flex-wrap gap-2 justify-center mb-5">
          {pool.map((w, i) => <button key={i} onClick={() => addWord(w, i)} className="px-4 py-2 rounded-xl text-sm font-bold hover:scale-105 active:scale-95" style={{ background: 'rgba(255,255,255,0.05)', color: '#e2e8f0', border: '1px solid rgba(255,255,255,0.1)' }}>{w}</button>)}
        </div>
        {pool.length === 0 && result === null && <button onClick={check} className="px-8 py-3 rounded-xl font-bold text-white hover:scale-[1.03]" style={{ background: 'linear-gradient(135deg,#22c55e,#16a34a)' }}>✓ Check</button>}
