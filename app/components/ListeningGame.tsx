...
export default function ListeningGame({ profile, userId, onFinish }: { profile: any; userId: string; onFinish: () => void }) {
  ...
  useEffect(() => { if (questions[idx]) setTimeout(() => speak(questions[idx].word), 400); }, [idx]);
    ...
    <div className="min-h-screen flex flex-col px-4 py-5" style={{ background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
        <button onClick={() => speak(q.word)}
          className="w-28 h-28 rounded-full flex items-center justify-center text-5xl mb-4 transition-all hover:scale-105 active:scale-95"
          style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', boxShadow: '0 0 40px rgba(99,102,241,0.4)' }}>🔊</button>
          ...
          {q.options.map((o: string, i: number) => {
            ...
            return (
              <button key={i} onClick={() => pick(o)} disabled={selected !== null}
                className="py-4 px-3 rounded-2xl font-bold text-sm transition-all hover:scale-[1.03] active:scale-[0.97] disabled:hover:scale-100"
