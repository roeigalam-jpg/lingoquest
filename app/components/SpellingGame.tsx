...
import { SPELLING_CONTENT as CONTENT } from '../lib/content';
...
export default function SpellingGame({ profile, userId, onFinish }: { profile: any; userId: string; onFinish: () => void }) {
  ...
  const submit = () => {
    ...
    setResult(ok);
    if (ok) { setStreak(s => s + 1); setScore(s => ({ ...s, c: s.c + 1 })); sounds.correct(); } else { setStreak(0); setScore(s => ({ ...s, w: s.w + 1 })); sounds.wrong(); sounds.speak(questions[idx].word); }
    setTimeout(() => { if (idx + 1 >= questions.length) { if (timerRef.current) clearInterval(timerRef.current); setDone(true); } else { setIdx(i => i + 1); setInput(''); setResult(null); } }, ok ? 700 : 1200);
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)' }}>
      <div className="w-full max-w-md"><div className="rounded-3xl p-7 text-center" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <div className="rounded-xl p-3 mb-4" style={{ background: 'linear-gradient(135deg,rgba(99,102,241,0.1),rgba(245,158,11,0.1))', border: '1px solid rgba(99,102,241,0.2)' }}>
          <div className="flex justify-center gap-5"><div><span>⭐</span><span className="font-black text-white ml-1">+{score.c * XP_PER}</span></div><div><span>💰</span><span className="font-black text-white ml-1">+{score.c * LINGO_PER}</span></div></div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-5" style={{ background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)' }}>
      <div className="max-w-md mx-auto w-full mb-4">
        ...
        <div className="flex items-center gap-2"><div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}><div className="h-full rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%`, background: 'linear-gradient(90deg,#f59e0b,#f97316)' }} /></div><span className="text-xs font-bold text-slate-400">{idx}/{questions.length}</span>{streak >= 3 && <span className="text-xs font-bold animate-pulse" style={{ color: '#fbbf24' }}>🔥{streak}</span>}</div>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
        ...
        <div className="w-full flex gap-2 mb-4">
          <input type="text" value={input} onChange={e => setInput(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && submit()} className="flex-1 px-4 py-3 rounded-xl text-center text-lg font-black tracking-widest" style={{ background: 'rgba(255,255,255,0.06)', border: result === true ? '2px solid #22c55e' : result === false ? '2px solid #ef4444' : '2px solid rgba(255,255,255,0.12)', color: '#e2e8f0', outline: 'none' }} placeholder="TYPE HERE" autoFocus />
