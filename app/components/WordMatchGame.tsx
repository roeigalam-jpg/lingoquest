...
import { WORD_MATCH_CONTENT as CONTENT } from '../lib/content';
...
export default function WordMatchGame({ profile, userId, onFinish }: { profile: any; userId: string; onFinish: () => void }) {
  ...
  useEffect(() => { if (questions[idx]) setTimeout(() => sounds.speak(questions[idx].word), 300); }, [idx, questions]);
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)' }}>
      <div className="w-full max-w-md"><div className="rounded-3xl p-7 text-center" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <div className="rounded-xl p-3 mb-4" style={{ background: 'linear-gradient(135deg,rgba(99,102,241,0.1),rgba(245,158,11,0.1))', border: '1px solid rgba(99,102,241,0.2)' }}>
          <div className="flex justify-center gap-5"><div><span>⭐</span><span className="font-black text-white ml-1">+{score.c * XP_PER}</span></div><div><span>💰</span><span className="font-black text-white ml-1">+{score.c * LINGO_PER}</span></div>{pct === 100 && <div><span>🎫</span><span className="font-black text-white ml-1">+1</span></div>}</div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-5" style={{ background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)' }}>
      <div className="max-w-md mx-auto w-full mb-4">
        ...
        <div className="flex items-center gap-2"><div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}><div className="h-full rounded-full transition-all duration-500" style={{ width: `${(idx / questions.length) * 100}%`, background: 'linear-gradient(90deg,#6366f1,#a78bfa)' }} /></div><span className="text-xs font-bold text-slate-400">{idx}/{questions.length}</span>{streak >= 3 && <span className="text-xs font-bold animate-pulse" style={{ color: '#fbbf24' }}>🔥{streak}</span>}</div>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
        <div className="relative mb-6">
          <div className="rounded-3xl flex items-center justify-center transition-all duration-300" style={{ width: 160, height: 160, background: 'rgba(255,255,255,0.05)', border: `2px solid ${correct === true ? '#22c55e' : correct === false ? '#ef4444' : 'rgba(255,255,255,0.1)'}`, transform: correct === true ? 'scale(1.05)' : correct === false ? 'scale(0.95)' : 'scale(1)' }}><span style={{ fontSize: 72 }}>{q.emoji}</span></div>
          <button onClick={() => sounds.speak(q.word)} className="absolute -bottom-3 -right-3 w-12 h-12 rounded-full flex items-center justify-center text-xl transition-all hover:scale-110" style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)' }}>🔊</button>
        </div>
        ...
        {correct !== null && <p className="mt-4 text-lg font-black" style={{ color: correct ? '#34d399' : '#f87171' }}>{correct ? (streak >= 3 ? '🔥 On Fire!' : 'Great! ⭐') : `Answer: ${q.word}`}</p>}
      </div>
    </div>
  );
}
